...
router.get('/', function(req, res) {
    ...
    conn.query('SELECT count(*) as cnt FROM tbl_product_coupon ' + addSQL + '; SELECT @rownum:=@rownum+1 as num,'
        + ' x.no as no, concat(LEFT(x.title,15),"...") as title, x.content as content, DATE_FORMAT(ins_dt, "%Y-%m-%d") as date, x.writer as writer,'
        + ' x.count as count, (select count(rno) from tbl_product_coupon_comment where bno = x.no) as rCount'
        + ' FROM tbl_product_coupon x, (SELECT @rownum:=' + skip + ') TMP' + addSQL
...
router.get('/new', function(req, res) {
    ...
    res.render('./products6/new', {title: '리뷰 작성 화면', session : ss});
});
...
router.post('/save', function(req, res) {
    ...
    conn.query('INSERT INTO tbl_product_coupon(title, content, writer, ins_dt, ins_id) VALUES(?, ?, ?, now(), ?)',
...
router.get('/view/:no', function(req, res) {
    ...
    // 뷰 카운트 추가.
    console.log("조회업데이트");
    conn.query('UPDATE tbl_product_coupon SET count = (select * from (select count+1 from tbl_product_coupon where no = ?) as tmp) WHERE no = ?',
        [no, no],
        function(err, results) {
            if (err) {
                console.log('error : ', err.message);
    ...
    console.log("뷰 조회.");
    // 조회.
    conn.query('select no, title, content, DATE_FORMAT(ins_dt, "%Y-%m-%d") as date, writer, count from tbl_product_coupon where no = ?;'
        + ' select count(*) cnt from tbl_product_coupon_comment where bno = ?;'
        + ' select rno as no, bno as bno, comment as comment, DATE_FORMAT(ins_dt,"%Y-%m-%d") as date, ins_id as id'
        + ' from tbl_product_coupon_comment where bno = ?;',
        ...
        function(err, results) {
            if (err) {
                ...
                console.log('routes product6 View !!!');

                res.render('./products6/view', {
                    result : results[0][0],
                    reply : results[1][0],
                    list : results[2],
                    session : ss
    ...
    conn.query('select no, title, content, DATE_FORMAT(ins_dt, "%Y-%m-%d") as date, writer, count from tbl_product_coupon where no = ?',
        ...
        function(err, results) {
            if(err) {
                ...
                console.log('routes product6 Edit Result !!!');
                res.render('./products6/edit', {title: '수정화면', result : results[0], session : ss});
            }
        });
});
...
router.post('/edit/do', function(req, res) {
    ...
    conn.query('UPDATE tbl_product_coupon SET title = ?, content = ?, writer = ?, upd_dt = now() upd_id = ? WHERE no = ?',
        ...
        function(err, results) {
            if(err) {
                ...
                res.redirect("/product6/view/"+req.body.no);
            }
        });
});
...
router.get('/delete/:no', function(req, res) {
    ...
    conn.query('DELETE FROM tbl_product_coupon WHERE no = ?', [no],
        function(err, results) {
            if(err) {
                console.log('error : ', err.message);
                res.render('error', {message: err.message, error : err, session: ss});
...
router.post('/delete', function(req, res) {
    ...
    for (var i = 0; i < params.length; i++) {
        ...
        conn.query('DELETE FROM tbl_product_coupon WHERE no = ?', [params[i]],
            function (err, results) {
                if (err) {
                    console.log('error : ', err.message);
                    res.render('error', {message: err.message, error : err, session: ss});
...
router.post('/comment/new', function(req, res) {
    ...
    // 전체 데이타를 조회한 후 결과를 'results' 매개변수에 저장.
    conn.query('INSERT INTO tbl_product_coupon_comment(bno, comment, ins_dt, ins_id) VALUES(?, ?, now(), ?)',
...
router.post('/comment/del', function(req, res) {
    ...
    // 삭제 처리.
    conn.query('DELETE FROM tbl_product_coupon_comment WHERE rno = ? AND bno = ? AND ins_id = ?',
        [rno, bno, uid],
        function(err, results) {
            if(err) {
                console.log('error : ', err.message);
