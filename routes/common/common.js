...
router.get('/', function(req, res) {
    console.log('공통처리 부분.');
    var ss = req.session;
    res.render('index', { title: 'adpay.co.kr', session : ss});
});
...
router.post('/idCheckProcess', function(req, res) {
    ...
    conn.query('select count(c_id) as cnt from tbl_user where c_id = ?',
        [req.body.usrId],
...
router.post('/emailCheckProcess', function(req, res) {
    ...
    conn.query('select count(c_email) as cnt from tbl_user where c_email = ?;',
        [req.body.usrEmail],
        function(err, results) {
            if(err) {
                console.log('error : ', err.message);
