...
router.get('/', function(req, res) {
    ...
    if(ss.usrLevel == '000' || ss.usrLevel == '001' || ss.usrLevel == '002') {
        ...
        conn.query('SELECT count(*) as cnt FROM tbl_coupon ' + addSQL + '; SELECT @rownum:=@rownum+1 as num, no,'
            + ' coupon_no as couponNo, coupon_status as couponStatus, DATE_FORMAT(ins_date, "%Y-%m-%d") as insDate,'
            + ' ins_usr_id as insUsrId, ins_usr_nm as insUsrNm FROM tbl_coupon, (SELECT @rownum:=' + skip + ') TMP' + addSQL
...
router.get('/use', function(req, res) {
    ...
    if(ss.usrLevel == '000' || ss.usrLevel == '001' || ss.usrLevel == '002') {
        ...
        // 전체 데이타를 조회한 후 결과를 'results' 매개변수에 저장.
        conn.query('select count(*) as cnt from tbl_coupon_use ' + addSQL + '; select @rownum:=@rownum+1 as num, no,'
            ...
            + ' DATE_FORMAT(accept_date, "%Y-%m-%d") as acceptDate from tbl_coupon_use, (SELECT @rownum:=' + skip + ') TMP' + addSQL
            + ' order by no desc limit ' + skip + ', ' + limit + ";",
            [srchText, srchText],
            function (err, results) {
                if (err) {
...
router.post('/insert', function(req, res) {
    ...
    conn.query('insert into tbl_coupon(title, content, date, writer) values(?, ?, now(), ?)',
...
router.get('/view/:no', function(req, res) {
    ...
    conn.query('SELECT coupon_no as couponNo, case when coupon_status = "NOTUSED" then "미사용" when coupon_status = "USED"'
        ...
        + ' FROM tbl_coupon WHERE coupon_no = ?',
        [req.params.no],
        function(err, results) {
            if (err) {
                console.log('error : ', err.message);
...
router.post('/edit/do', function(req, res) {
    ...
    conn.query('update tbl_coupon set coupon_status = ? where coupon_no = ?',
...
router.get('/delete/:no', function(req, res) {
    ...
    conn.query('delete from tbl_coupon where coupon_no = ?',
        [req.params.no],
        function(err) {
            if(err) {
                console.log('error : ', err.message);
...
router.get('/use/view/:no', function(req, res) {
    ...
    conn.query('SELECT coupon_no as couponNo, ins_usr_id as insUsrId, ins_usr_nm as insUsrNm,'
        ...
        + ' IFNULL(DATE_FORMAT(accept_date, "%Y-%m-%d %H:%m"),"") as acceptDate FROM tbl_coupon_use WHERE coupon_no = ?',
        [req.params.no],
        function(err, results) {
            if (err) {
                console.log('error : ', err.message);
...
router.get('/use/edit/:no', function(req, res) {
    ...
    conn.query('SELECT coupon_no as couponNo, ins_usr_id as insUsrId, ins_usr_nm as insUsrNm,'
        ...
        + ' FROM tbl_coupon_use WHERE coupon_no = ?',
        [req.params.no],
        function(err, results) {
            if (err) {
                console.log('error : ', err.message);
...
router.post('/use/edit/do', function(req, res) {
    ...
    console.log("게시글 수정 처리");
    conn.query('update tbl_coupon_use set accept_yn = ?, accept_usr_id =?, accept_usr_nm = ?, accept_date = now() where coupon_no = ?',
...
router.get('/use/delete/:no', function(req, res) {
    ...
    // 전체 데이타를 조회한 후 결과를 'results' 매개변수에 저장.
    console.log("게시글 삭제 처리");
    conn.query('delete from tbl_coupon_use where coupon_no = ?',
        [req.params.no],
        function(err) {
            if(err) {
                console.log('error : ', err.message);
