...
router.get('/', function(req, res) {
    ...
    if(ss.usrLevel == '000' || ss.usrLevel == '001' || ss.usrLevel == '002') {
        ...
        conn.query('SELECT count(*) as cnt FROM tbl_product x WHERE x.p_type = "MC"' + addSQL
            + '; SELECT @rownum:=@rownum+1 as num, x.p_no as pNo, x.p_code as pCode, x.p_nm as pNm,'
            + ' (select comm_nm from tbl_commcd where p_comm_cd = "PT00" and comm_cd = x.p_type) as pType,'
            + ' concat(FORMAT(x.p_price,0), " (원)") as pPrice, set_best_yn as setBestYn,'
            + ' case when x.p_display_yn = "y" then "예" when x.p_display_yn = "n" then "아니오" else "" end as displayYn,'
            + ' DATE_FORMAT(x.insert_dt, "%Y-%m-%d") as wDt,'
            + ' insert_usr as wNm, DATE_FORMAT(x.update_dt, "%Y-%m-%d") as uDt,'
            + ' update_usr as uNm FROM tbl_product x, (SELECT @rownum:=' + skip + ') TMP WHERE x.p_type = "MC"' + addSQL
            ...
            function (err, results) {
                if (err) {
                    ...
                    res.render('./admin/product6/list', {
                        product: results[1],
                        srchType: srchType,
                        srchText: srchText,
                        page: page,
    ...
    conn.query('SELECT count(*) as cnt from tbl_product x WHERE x.p_type = "MC"' + addSQL
        ...
        function(err, results) {
            if(err) {
                ...
                res.render('./admin/product6/list', {product : results[1], srchType: srchType, srchText : srchText, page : page, maxPage: maxPage, offset: offset, session : ss});
            }
        });
...
router.get('/new', function(req, res) {
    ...
    conn.query('SELECT @rownum:=@rownum+1 as num, s.cateNo as cateNo, s.cateName as cateName FROM (SELECT t1.category_no as cateNo,'
        ...
        function(err, results) {
            if(err) {
                ...
                var list = results != null ? results : '';

                res.render('./admin/product6/new', {'cList2' : list, 'session' : ss});
            }
        });
});
...
router.post('/save', function(req, res) {
    ...
    conn.query('INSERT INTO tbl_product(p_nm, p_code, p_div, p_type, p_price, p_disc_price,'
        + ' p_smmry, p_desc, p_display_yn, p_image, p_image_url, p_stock_count,'
        + ' display_order_no, set_best_yn, insert_dt, insert_usr, update_dt, update_usr)'
        + ' VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, now(), ?, now(), ?)',
        [
            pNm, pCode, pDiv, pType, pPrice, pDiscPrice, pSmmry, pDesc, pDisplayYn, pImage,
            pImageUrl, pStockCnt, displayOrderNo, pSetBestYn, ssId, ssId
        ],
        function(err) {
            if(err) {
                console.log('error : ', err.message);
...
router.get('/edit/:no', function(req, res) {
    ...
    conn.query('SELECT p_no as pNo, p_code as pCode, p_div as pDiv, p_nm as pNm,'
        + ' p_price as pPrice, p_disc_price as pDiscPrice, p_smmry as pSmmry, p_desc as pDesc, p_type as pType, p_display_yn as pDisplayYn,'
        + ' p_image as pImage, p_image_url as pImageUrl, display_order_no as pDispOrderNo, set_best_yn as setBestYn,'
        + ' category1 as pCate1, category2 as pCate2, category3 as pCate3, p_stock_count as pStockCount,'
        + ' DATE_FORMAT(insert_dt, "%Y-%m-%d") as w_dt, insert_usr as w_nm, DATE_FORMAT(update_dt, "%Y-%m-%d") as u_dt,'
        + ' update_usr as u_nm FROM tbl_product WHERE p_type = "MC" AND p_no = ?;'
        ...
        function(err, results) {
            if(err) {
                ...
                res.render('./admin/product6/edit', {'result' : results[0][0], 'cList2' : results[1], 'session' : ss});
            }
        });
});
...
router.get('/view/:no', function(req, res) {
    ...
    conn.query('SELECT p_no as pNo, p_code as pCode, p_div as pDiv, p_nm as pNm, p_price as pPrice, p_smmry as pSmmry, p_desc as pDesc,'
        + ' p_display_yn as pDisplayYn, p_image as pImage,'
        + ' DATE_FORMAT(insert_dt, "%Y-%m-%d") as w_dt, insert_usr as w_nm, DATE_FORMAT(update_dt, "%Y-%m-%d") as u_dt,'
        + ' update_usr as u_nm FROM tbl_product WHERE p_type = "MC" AND p_no = ?;',
        [req.params.pNo],
        function(err, results) {
            if(err) {
                ...
                res.render('./admin/product6/view', {result : results[0], session : ss});
            }
        });
});
...
router.post('/edit/do', function(req, res) {
    ...
    conn.query('UPDATE tbl_product SET p_code = ?, p_div = ?, p_type = ?, p_nm = ?, p_price = ?, p_disc_price = ?,'
        + ' p_smmry = ?, p_desc = ?, p_display_yn = ?, set_best_yn = ?, p_image = ?,'
        + ' p_image_url = ?, display_order_no = ?, p_stock_count = ?,'
        + ' insert_dt = now(), insert_usr = ?, update_dt = now(), update_usr = ? WHERE p_type = "MC" AND p_no = ?',
        [
            pCode, pDiv, pType, pNm, pPrice, pDiscPrice, pSmmry, pDesc, pDisplayYn, pSetBestYn, pImage, pImageUrl,
            displayOrderNo, pStockCnt, ss.usrId, ss.usrId, pNo
        ],
        function(err, results) {
            if(err) {
                console.log('error : ', err.message);
...
router.get('/delete/:no', function(req, res) {
    ...
    var ss = req.session;
    var pNo = req.params.no != null ? req.params.no : '';

    // 전체 데이타를 조회한 후 결과를 'results' 매개변수에 저장.
    console.log("게시글 삭제 처리");
    conn.query('DELETE FROM tbl_product WHERE p_type = "MC" AND p_no = ?',
        [pNo],
        function(err, results) {
            if(err) {
                console.log('error : ', err.message);
...
router.get('/banner', function(req, res, next) {
    ...
    conn.query(SQL, [], function(err, results) {
        if(err) {
            ...
            res.status(200).render('./admin/product6/banner/view', {'result' : results[0], 'session' : ss});
        }
    })
...
router.get('/banner/edit', function(req, res, next) {
    ...
    conn.query(SQL, [], function(err, results) {
        if(err) {
            ...
            console.log('call banner ');
            console.log('results : ', JSON.stringify(results) + '\n');
            res.status(200).render('./admin/product6/banner/edit', {'result' : results[0], 'session' : ss});
        }
    })
...
router.post('/getDisplayNoYn', function(req, res) {
    var ss = req.session;
    var displayOrderNo = req.body.pDispOrderNo !=null ? req.body.pDispOrderNo : '';
    var SQL = 'SELECT p_code as pCode, p_nm as pNm FROM tbl_product WHERE p_type = "MC"'
        + ' AND p_div = "M" AND  p_display_yn = "Y" AND display_order_no = ?';
...
router.post('/getUniqCdUseYn', function(req, res) {
    ...
    var SQL = 'SELECT p_code as pCode, p_nm as pNm, p_uniq_code as pUniqCode FROM tbl_product'
        + ' WHERE p_type = "MC" AND p_div = "M" AND p_uniq_code = ?';

    conn.query(SQL, [pUniqCd],
        function(err, results) {
            if(err) {
                console.log('err : ', err);
            } else {
    ...
    conn.query('SELECT @rownum:=@rownum+1 as num, s.cateNo, s.cateName from (SELECT t1.category_no as cateNo,'
        ...
        function(err, results) {
            if(err) {
                ...
                res.render('./admin/product6/category/list', {rList : results, session : ss});
            }
        }
    );
    ...
    conn.query('INSERT INTO tbl_product_category(`name`) values( ? );',
        ...
        function(err) {
            if(err) {
                ...
                res.redirect('/admin/product6/category');
            }
        });
});
    ...
    conn.query('SELECT @rownum:=@rownum+1 as num, s.cateName1, s.cateName2, s.cateNo FROM ('
        ...
        function(err, results) {
            if(err) {
                ...
                res.render('./admin/product6/category/listM', {rList : results[0], rList2 : results[1], session : ss});
            }
        }
    );
    ...
    conn.query('INSERT INTO tbl_product_category(`name`, `parent_no`) values(?, ?);',
        ...
        function(err) {
            if(err) {
                ...
                res.redirect('/admin/product6/category2');
            }
        });
});
    ...
    conn.query('SELECT @rownum:=@rownum+1 as num, s.cateName1, s.cateName2, s.cateName3, s.cateNo FROM ('
        ...
        function(err, results) {
            if(err) {
                ...
                res.render('./admin/product6/category/listS', {rList : results[0], rList2 : results[1], cList : results[2], srchCategory1 : srchCategory1 , srchCategory2 : srchCategory2, session : ss});
            }
        }
    );
    ...
    conn.query('INSERT INTO tbl_product_category(`name`, `parent_no`) values(?, ?);',
        ...
        function(err) {
            if(err) {
                ...
                res.redirect('/admin/product6/category3');
            }
        });
});
...
router.post('/best/save', function(req, res) {
    ...
    for (var i = 0; i < params.length; i++) {
        console.log(">>> get params[" + i + "] =" + params[i]);
        conn.query('UPDATE tbl_product SET set_best_yn = "Y" WHERE p_type = "MC" AND p_no = ?',
            [params[i]],
            function (err, results) {
                if (err) {
                    console.log('error : ', err.message);
