...
var setPath = './public/images/product';
...
router.get('/', function(req, res) {
    ...
    if(ss.usrLevel == '000' || ss.usrLevel == '001' || ss.usrLevel == '002') {
        ...
        conn.query('SELECT count(*) as cnt from tbl_product x'
            + ' WHERE x.category1 = (SELECT s.cateNo as cateNo FROM (SELECT t1.category_no as cateNo,'
            + '  t1.name as cateName FROM tbl_product_category AS t1'
            + ' WHERE t1.parent_no is null GROUP BY t1.name ORDER BY t1.name ASC) s'
            + ' WHERE s.cateName = "온라인게임")' + addSQL
            + '; SELECT @rownum:=@rownum+1 as num, x.p_no as pNo, x.p_code as pCode, x.p_nm as pNm,'
            + ' (select comm_nm from tbl_commcd where p_comm_cd = "P000" and comm_cd = x.p_type) as pType,'
            + ' concat(FORMAT(x.p_price,0), " (원)") as pPrice, x.p_disc_price as pDiscPrice, x.p_disc_percent as pDiscPercent,'
            + ' case when x.p_display_yn = "y" then "예" when x.p_display_yn = "n" then "아니오" else "" end as displayYn,'
            + ' (select b.comm_nm from tbl_commcd b where b.p_comm_cd = "SD00" and b.comm_cd = x.p_gubun_cd) as srcGubun,'
            + ' x.sort_no as sortNo, x.set_best_yn as setBestYn, DATE_FORMAT(x.insert_dt, "%Y-%m-%d") as wDt,'
            + ' insert_usr as wNm, DATE_FORMAT(x.update_dt, "%Y-%m-%d") as uDt,'
            + ' update_usr as uNm FROM tbl_product x, (SELECT @rownum:=' + skip + ') TMP'
            + ' WHERE x.category1 = (SELECT s.cateNo as cateNo FROM (SELECT t1.category_no as cateNo,'
            + '  t1.name as cateName FROM tbl_product_category AS t1'
            + ' WHERE t1.parent_no is null GROUP BY t1.name ORDER BY t1.name ASC) s'
            + ' WHERE s.cateName = "온라인게임")' + addSQL
            ...
            function (err, results) {
                if (err) {
                    ...
                    res.render('./admin/product2/list', {
                        product: results[1],
                        srchType: srchType,
                        srchText: srchText,
                        page: page,
...
router.get('/new', function(req, res) {
    ...
    conn.query('SELECT comm_cd as commCd, comm_nm as commNm FROM tbl_commcd WHERE p_comm_cd = "SD00";'
        ...
        function(err, results) {
            if(err) {
                ...
                res.render('./admin/product2/new', {'cList' : results[0], 'cList2' : results[1], 'session' : ss});
            }
        });
});
...
router.post('/insert', function(req, res) {
    ...
    var pDiscPrice = req.body.pDiscPrice !=null ? req.body.pDiscPrice : 0;
    var pDiscPercent = req.body.pDiscPercent !=null ? req.body.pDiscPercent : 0;
    ...
    var pSetBestYn = req.body.pSetBestYn !=null ? req.body.pSetBestYn : '';

    conn.query('insert into tbl_product(p_nm, p_code, p_div, p_opt_btn_dis_yn, p_type, p_gubun, p_gubun_cd, p_price,'
        + ' p_disc_price, p_disc_percent, p_smmry, p_desc, p_display_yn, p_image, p_image_url, p_uniq_code, category1, category2, category3,'
        + ' p_stock_count, set_best_yn, sort_no, display_order_no, insert_dt, insert_usr, update_dt, update_usr)'
        + ' values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, now(), ?, now(), ?)',
        [pNm, pCode, pDiv, optBtnDispYn, pType, pGubun, pGubunCd, pPrice, pDiscPrice, pDiscPercent, pSmmry, pDesc, pDisplayYn, pImage,
            pImageUrl, pUniqCode, category1, category2, category3, pStockCnt, pSetBestYn, sortNo, displayOrderNo, ss.usrId, ss.usrId],
        function(err, results) {
            if(err) {
                console.log('error : ', err.message);
                res.render('error', {message: err.message, error : err, session: ss});
...
router.get('/edit/:no', function(req, res) {
    ...
    conn.query('SELECT p_no as pNo, p_code as pCode, p_div as pDiv, p_opt_btn_dis_yn as pOptBtnDisYn, p_nm as pNm,'
        + ' p_price as pPrice, p_disc_price as pDiscPrice, p_disc_percent as pDiscPercent, p_smmry as pSmmry,'
        + ' p_desc as pDesc, p_gubun as pGubun, p_gubun_cd as pGubunCd, p_type as pType, p_display_yn as pDisplayYn,'
        ...
        + ' display_order_no as pDispOrderNo, category1 as pCate1, category2 as pCate2, category3 as pCate3, p_stock_count as pStockCount, set_best_yn as setBestYn,'
        + ' DATE_FORMAT(insert_dt, "%Y-%m-%d") as w_dt, insert_usr as w_nm, DATE_FORMAT(update_dt, "%Y-%m-%d") as u_dt,'
        + ' update_usr as u_nm FROM tbl_product WHERE p_no = ?;'
        + ' select comm_cd as commCd, comm_nm as commNm from tbl_commcd where p_comm_cd = "SD00";'
        + ' SELECT @rownum:=@rownum+1 as num, s.cateNo as cateNo, s.cateName as cateName FROM (SELECT t1.category_no as cateNo,'
        + ' t1.name as cateName FROM tbl_product_category AS t1'
        + ' WHERE t1.parent_no is null GROUP BY t1.name ORDER BY t1.name ASC) s, (SELECT @rownum:=0) TMP;',
        [no],
        function(err, results) {
            if(err) {
                ...
                res.render('./admin/product2/edit', {'result' : results[0][0], 'cList' : results[1], 'cList2' : results[2], 'session' : ss});
            }
        });
});
...
router.get('/view/:no', function(req, res) {
    ...
    conn.query('SELECT p_no as pNo, p_code as pCode, p_div as pDiv, p_nm as pNm, p_price as pPrice, p_smmry as pSmmry, p_desc as pDesc,'
        ...
        + ' DATE_FORMAT(insert_dt, "%Y-%m-%d") as w_dt, insert_usr as w_nm, DATE_FORMAT(update_dt, "%Y-%m-%d") as u_dt,'
        + ' update_usr as u_nm FROM tbl_product WHERE category1 = "온라인게임" AND p_no = ?;'
        + ' select comm_cd as commCd, comm_nm as commNm from tbl_commcd where p_comm_cd = "SD00";',
        [pNo],
        function(err, results) {
            if(err) {
                ...
                res.render('./admin/product2/view', {result : results[0], cList : results[1], session : ss});
            }
        });
});
...
router.post('/edit/do', function(req, res) {
    ...
    conn.query('UPDATE tbl_product SET p_code = ?, p_pair_code_yn = ?, p_pair_code = ?, p_div = ?,'
        + ' p_opt_btn_dis_yn = ?, p_type = ?, p_gubun = ?, p_gubun_cd = ?, p_nm = ?, p_price = ?, p_disc_price = ?,'
        + ' p_disc_percent = ?, p_smmry = ?, p_desc = ?, p_display_yn = ?, p_image = ?, p_image_url = ?, p_uniq_code = ?,'
        + ' sort_no = ?, display_order_no = ?, category1 = ?, category2 = ?, category3 = ?, p_stock_count = ?, set_best_yn = ?,'
        + ' insert_dt = now(), insert_usr = ?, update_dt = now(), update_usr = ? WHERE p_no = ?',
        [
            pCode, pPaireCodeYn, pPaireCode, pDiv, optBtnDispYn, pType, pGubun, pGubunCd, pNm, pPrice, pDiscPrice,
            pDiscPercent, pSmmry, pDesc, pDisplayYn, pImage, pImageUrl, pUniqCode, sortNo, displayOrderNo, category1,
            category2, category3, pStockCnt, pSetBestYn, ss.usrId, ss.usrId, pNo
        ],
        function(err, results) {
            if(err) {
                console.log('error : ', err.message);
...
router.get('/delete/:no', function(req, res) {
    ...
    conn.query('DELETE FROM tbl_product WHERE category1 = "온라인게임" AND p_no = ?',
        [pNo],
        function(err, results) {
            if(err) {
                console.log('error : ', err.message);
...
router.post('/delete', function(req, res) {
    ...
    for (var i = 0; i < params.length; i++) {
        ...
        conn.query('DELETE FROM tbl_product WHERE p_no = ?',
            [params[i]],
            function (err, results) {
                if (err) {
                    console.log('error : ', err.message);
...
router.get('/banner', function(req, res, next) {
    ...
    conn.query(SQL, [], function(err, results) {
        if(err) {
            ...
            res.status(200).render('./admin/product2/banner/view', {'result' : results[0], 'session' : ss});
        }
    })
...
router.get('/banner/edit', function(req, res, next) {
    ...
    conn.query(SQL, [], function(err, results) {
        if(err) {
            ...
            console.log('call banner ');
            console.log('results : ', JSON.stringify(results) + '\n');
            res.status(200).render('./admin/product2/banner/edit', {'result' : results[0], 'session' : ss});
        }
    })
...
router.post('/getDisplayNoYn', function(req, res) {
    var ss = req.session;
    var displayOrderNo = req.body.pDispOrderNo !=null ? req.body.pDispOrderNo : '';
    var SQL = 'SELECT p_code as pCode, p_nm as pNm FROM tbl_product WHERE p_div = "M" AND  p_display_yn = "Y"'
...
router.post('/getUniqCdUseYn', function(req, res) {
    ...
    var SQL = 'SELECT p_code as pCode, p_nm as pNm, p_uniq_code as pUniqCode FROM tbl_product WHERE p_div = "M"'
        + ' AND p_uniq_code = ?';
    ...
    conn.query('SELECT @rownum:=@rownum+1 as num, s.cateNo, s.cateName from (SELECT t1.category_no as cateNo,'
        ...
        function(err, results) {
            if(err) {
                ...
                res.render('./admin/product2/category/list', {rList : results, session : ss});
            }
        }
    );
...
router.post('/category/insert', function(req, res) {
    ...
    conn.query('INSERT INTO tbl_product_category(`name`) values( ? );',
        [req.body.cateName],
        function(err) {
            if(err) {
                console.log('error : ', err.message);
                ...
                res.redirect('/admin/product2/category');
            }
        });
});
...
router.post('/category/delete', function(req, res) {
    ...
    for (var i = 0; i < params.length; i++) {
        ...
        conn.query('delete from tbl_product_category where category_no = ?;'
            + ' delete from tbl_product_category where parent_no = ?;',
            [params[i], params[i]],
            function (err) {
                if (err) {
                    console.log('error : ', err.message);
...
router.get('/category2', function(req, res) {
    ...
    conn.query('SELECT @rownum:=@rownum+1 as num, s.cateName1, s.cateName2, s.cateNo FROM ('
        + 'SELECT t1.name as cateName1, t2.name as cateName2, t2.category_no as cateNo FROM tbl_product_category AS t1'
        + ' LEFT JOIN tbl_product_category as t2 ON t1.category_no = t2.parent_no WHERE t1.parent_no is null'
        + ' group by t1.name, t2.name order by t1.name, t1.order_no, t2.order_no asc) s, (SELECT @rownum:=0) TMP;'
        + 'SELECT t3.category_no as cateNo3, t3.name as cateName3 FROM tbl_product_category t3 WHERE t3.parent_no is null'
        ...
        function(err, results) {
            if(err) {
                ...
                res.render('./admin/product2/category/listM', {rList : results[0], rList2 : results[1], session : ss});
            }
        }
    );
...
router.post('/category2/insert', function(req, res) {
    ...
    conn.query('INSERT INTO tbl_product_category(`name`, `parent_no`) values(?, ?);',
        [cateName, cateSelBox],
        function(err) {
            if(err) {
                console.log('error : ', err.message);
                ...
                res.redirect('/admin/product2/category2');
            }
        });
});
...
router.get('/category3', function(req, res) {
    ...
    conn.query('SELECT @rownum:=@rownum+1 as num, s.cateName1, s.cateName2, s.cateName3, s.cateNo FROM ('
        ...
        + ' FROM tbl_product_category AS t1 LEFT JOIN tbl_product_category AS t2 ON t2.parent_no = t1.category_no'
        + ' LEFT JOIN tbl_product_category AS t3 ON t3.parent_no = t2.category_no WHERE t1.parent_no is null'
        + ' group by t1.name, t2.name, t3.name order by t1.name, t2.order_no, t3.order_no) s, (SELECT @rownum:=0) TMP;'
        + ' SELECT t2.name as cateName3, t2.category_no as cateNo3 FROM tbl_product_category AS t1'
        + ' LEFT JOIN tbl_product_category as t2 ON t1.category_no = t2.parent_no WHERE t2.category_no IS NOT NULL;'
        + ' SELECT t1.category_no as cateNo, t1.name as cateName FROM tbl_product_category AS t1'
        ...
        function(err, results) {
            if(err) {
                ...
                res.render('./admin/product2/category/listS', {rList : results[0], rList2 : results[1], cList : results[2], srchCategory1 : srchCategory1 , srchCategory2 : srchCategory2, session : ss});
            }
        }
    );
    ...
    conn.query('INSERT INTO tbl_product_category(`name`, `parent_no`) values(?, ?);',
        ...
        function(err) {
            if(err) {
                ...
                res.redirect('/admin/product2/category3');
            }
        });
});
...
router.post('/getcate2', function(req, res) {
    ...
    conn.query('SELECT t2.name as cateName2, t2.category_no as cateNo2 FROM tbl_product_category AS t1'
        + ' LEFT JOIN tbl_product_category AS t2 ON t2.parent_no = t1.category_no WHERE t1.category_no = ?;',
        [getDataVal],
        function(err, results) {
            if(err) {
                console.log('error : ', err.message);
...
router.post('/getcate3', function(req, res) {
    ...
    conn.query('SELECT t3.name as cateName3, t3.category_no as cateNo3 FROM tbl_product_category AS t2'
        + ' LEFT JOIN tbl_product_category AS t3 ON t3.parent_no = t2.category_no WHERE t2.category_no = ?;',
        [getDataVal],
        function(err, results) {
            if(err) {
                console.log('error : ', err.message);
 ...
 * 베스트 상품 등록 처리.
 */
router.post('/best/save', function(req, res) {

    // 전체 데이타를 조회한 후 결과를 'results' 매개변수에 저장.
    console.log("베스트 상품 등록 처리");
    ...
    for (var i = 0; i < params.length; i++) {
        console.log(">>> get params[" + i + "] =" + params[i]);
        conn.query('UPDATE tbl_product SET set_best_yn = "Y" WHERE p_no = ?',
            [params[i]],
            function (err) {
                if (err) {
                    console.log('error : ', err.message);
                    res.status(400).json({message: err.message, error : err, session: ss});
                }
            });
        conn.commit();
    }
