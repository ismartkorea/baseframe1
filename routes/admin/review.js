...
router.get('/new', function(req, res) {
    ...
    res.render('./admin/review/new', {title: '리뷰 작성 화면', session : ss});
});
    ...
    conn.query('select no, title, content, DATE_FORMAT(ins_dt, "%Y-%m-%d") as date, writer, count from tbl_review where no = ?',
        ...
        function(err, results) {
            if(err) {
                ...
                res.render('./admin/review/edit', {title: '수정화면', result : results[0], session : ss});
            }
        });
});
...
router.post('/edit/do', function(req, res) {
    ...
    conn.query('UPDATE tbl_review SET title = ?, content = ?, writer = ?, ins_dt = now(), ins_id = ? WHERE no = ?',
        ...
        function(err) {
            if(err) {
                ...
                res.redirect("/admin/review/view/"+no);
            }
        });
});
...
router.post('/delete', function(req, res) {
    ...
    for (var i = 0; i < params.length; i++) {
        ...
        conn.query('DELETE FROM tbl_review WHERE no = ?',
            [params[i]],
            function (err) {
                if (err) {
                    console.log('error : ', err.message);
