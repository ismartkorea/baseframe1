...
router.get('/', function(req, res) {
    ...
    if(ss.usrLevel == '000' || ss.usrLevel == '001' || ss.usrLevel == '002') {
        ...
        conn.query('SELECT count(*) as cnt FROM tbl_announce' + addSQL
            + '; SELECT @rownum:=@rownum+1 as num, no as no, title as title, content as content, writer as writer,'
            + ' cate_cd as cateCd, cate_nm as cateNm, DATE_FORMAT(ins_dt, "%Y-%m-%d") as date, ins_id as writer,'
