...
router.get('/', function(req, res) {
    ...
    if(ss.usrLevel == '000' || ss.usrLevel == '001' || ss.usrLevel == '002') {
        ...
        conn.query('select count(*) as cnt from tbl_user ' + addSQL1
            + '; select @rownum:=@rownum+1 as num, x.c_no as no, x.c_id as id, AES_DECRYPT(UNHEX(x.c_pwd),"jtlab") as pwd, x.c_name as name,'
            + ' x.c_addr1 as address1, x.c_addr2 as address2, x.c_postno as postno,'
            + ' x.c_email as email, x.c_sex as sex, x.c_birth as birth, x.c_jumin_no as juminno, x.c_tel_no as telno,'
            + ' x.c_tel_no1 as telno1, x.c_tel_no2 as telno2, x.c_tel_no3 as telno3, x.c_cell_no as cellno,'
            + ' x.c_cell_no1 as cellno1, x.c_cell_no2 as cellno2, x.c_cell_no3 as cellno3, x.c_comp_nm as compnm,'
            + ' x.c_comp_tel_no as comptelno, x.c_comp_tel_no1 as comptelno1, x.c_comp_tel_no2 as comptelno2, x.c_comp_tel_no3 as comptelno3,'
            + ' x.c_comp_addr1 as compaddr1, x.c_comp_addr2 as compaddr2, x.c_comp_postno as comppostno, x.c_saup_no as saupno, x.c_email_yn as emailyn,'
            + ' (select comm_nm from commcd_inf_tbl where p_comm_cd = "U000" and comm_cd = x.c_user_tp) as usertype, x.c_inf_use_yn as infoyn,'
...
router.post('/search', function(req, res) {
    ...
    conn.query('select count(*) as cnt from tbl_user '+ addSQL1
        + '; select @rownum:=@rownum+1 as num, x.c_no as no, x.c_id as id, AES_DECRYPT(UNHEX(x.c_pwd),"jtlab") as pwd, x.c_name as name,'
        + ' x.c_addr1 as address1, x.c_addr2 as address2, x.c_postno as postno,'
        + ' x.c_email as email, x.c_sex as sex, x.c_birth as birth, x.c_jumin_no as juminno, x.c_tel_no as telno, x.c_cell_no as cellno,'
        + ' x.c_comp_nm as compnm, x.c_comp_tel_no as comptelno, x.c_comp_addr1 as compaddr1, x.c_comp_addr2 as compaddr2,'
        + ' x.c_comp_postno as comppostno, x.c_saup_no as saupno, x.c_email_yn as emailyn,'
        + ' (select comm_nm from commcd_inf_tbl where p_comm_cd = "U000" and comm_cd = x.c_user_tp) as usertype, x.c_inf_use_yn as infoyn,'
        + ' (select comm_nm from commcd_inf_tbl where p_comm_cd = "M000" and comm_cd = x.c_auth_level) as level, x.c_auth_cd as authcd'
        + ' from tbl_user x, (SELECT @rownum:='+skip+') TMP' + addSQL2
        + ' order by x.c_no desc limit '+ skip + ', ' + limit + ";",
        ...
        function(err, results) {
            if(err) {
                ...
                res.render('./admin/member/list', {members : results[1], srchType: srchType, srchText : srchText, page : page, maxPage: maxPage, offset: offset, session : ss});
            }
        });
...
router.get('/view/:no', function(req, res) {
    ...
    //conn.query('select no, id, pwd, name, email, sex, age, address1, address2, postno, hobby, married, carno1, carno2, carno3, carlicen, usertype, saupno, juminno, infoyn, membertype from tbl_user where no = ?',
    conn.query('select c_no as no, c_id as id, AES_DECRYPT(UNHEX(c_pwd),"jtlab") as pwd, c_name as name, c_addr1 as address1, c_addr2 as address2, c_postno as postno, c_email as email, c_sex as sex, c_saup_no as saupno, c_user_tp as usertype, c_auth_level as level, c_birth as birth, c_auth_cd as authcd, c_jumin_no as juminno, c_inf_use_yn as infoyn from tbl_user where c_no = ?',
        [req.params.no],
        function(err, results) {
            if(err) {
                console.log('error : ', err.message);
                ...
                res.render('./admin/member/view', {result : 'OK', members : results[0], session : ss});
            }
        });
});
    ...
    conn.query('select c_no as no, c_id as id, AES_DECRYPT(UNHEX(c_pwd),"jtlab") as pwd, c_name as name,'
        ...
        function(err, results) {
            if(err) {
                ...
                res.render('./admin/member/edit', {result : 'OK', members : results[0], session : ss});
            }
        });
});
...
router.post('/edit', function(req, res) {
    ...
    conn.query('UPDATE tbl_user SET c_id = ?, c_pwd = HEX(AES_ENCRYPT(?,"jtlab")), c_name = ?,'
...
router.get('/delete/:no', function(req, res) {
    ...
    conn.query('select @rownum:=@rownum+1 as num, c_no as no, c_id as id, AES_DECRYPT(UNHEX(c_pwd),"jtlab") as pwd, c_name as name, c_addr1 as address1, c_addr2 as address2, c_postno as postno, c_email as email, c_sex as sex, c_bus_no as saupno, c_meb_typ as usertype, c_auth_level as level, c_birth as birth, c_auth_cd as authcd, c_jumin_no as juminno, c_inf_use_yn as infoyn from tbl_user, (SELECT @rownum:=0) TMP',
        [],
        function(err, results) {
            if(err) {
                console.log('error : ', err.message);
            } else {
                console.log('routes member list Result !!!');
                res.render('./admin/member/list', {board : results, session : ss});
            }
        });
});
