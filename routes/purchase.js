...
router.get('/:pCode/:pUniqCode', function(req, res) {
    ...
    // 상품 리스트 조회.
    var SQL1 = 'SELECT x.p_no as pNo, x.p_code as pCode, x.p_nm as pNm, x.p_price as pPrice, x.p_uniq_code as pUniqCode, x.p_smmry as pSmmry, x.category1 as cateNo,'
        + ' (SELECT s.cateName as cateNm FROM (SELECT t1.category_no as cateNo, t1.name as cateName FROM tbl_product_category AS t1'
        + ' WHERE t1.parent_no is null GROUP BY t1.name ORDER BY t1.name ASC) s WHERE s.cateNo = x.category1) as cateNm,'
        + ' x.p_disc_price as pDiscPrice, x.p_image as pImage, x.p_desc as pDesc, x.p_type as pType, x.opt_cnt as optCnt, x.p_stock_count as pStockCount'
        + ' FROM tbl_product x WHERE x.p_div = "M" AND x.p_display_yn = "Y" AND x.p_code = ?;';
    // 장바구니 리스트 조회.
    var SQL2 = ' SELECT p_code as pCode, p_pair_code_yn as pPairCodeYn, p_pair_code as pPairCode, p_div as pDiv, p_type as pType,'
        ...
        + ' p_uniq_code as pUniqCode FROM tbl_cart WHERE usr_id = ? AND p_uniq_code = ? ORDER BY p_code ASC, sort_no ASC;';
    // 장바구니 합계 조회.
    var SQL3 = 'SELECT SUM(p_price) as price FROM tbl_cart WHERE usr_id = ? AND p_uniq_code = ?;';
    // 상품결제 정보 조회.
    var SQL4 = 'SELECT x.order_no as orderNo, x.p_code as pCode, x.p_pair_code_yn as pPairCodeYn,'
        ...
        + ' FROM tbl_order_detail x, tbl_pay y WHERE x.order_no = y.order_no AND x.p_type = "SVC"'
    ...
    var SQL5 = 'SELECT x.order_no as orderNo, x.use_term_days as useTermDays, @rDays:=use_term_days, DATEDIFF(now(), x.pay_date) as usedDays,'
        ...
        + ' FROM tbl_pay x, tbl_order_detail y WHERE x.order_no = y.order_no AND y.p_type = "SVC" AND y.p_code = ? AND x.pay_result = "paid" AND x.usr_id = ?;';
    // 결제 정보 승인대기 갯수 조회.
    var SQL6 = ' SELECT COUNT(x.order_no) as cnt'
        + ' FROM tbl_pay x, tbl_order_detail y WHERE x.order_no = y.order_no'
...
router.get('/', function(req, res) {
    ...
    conn.query('SELECT p_no as pNo, p_code as pCode, p_nm as pNm, p_price as pPrice, p_uniq_code as pUniqCode,'
        + ' p_desc as pDesc, opt_cnt as optCnt FROM tbl_product WHERE p_div = "M" AND p_display_yn = "Y";'
        ...
        + ' p_uniq_code as pUniqCode FROM tbl_cart WHERE usr_id = ? ORDER BY p_code ASC, sort_no ASC;'
        + 'SELECT SUM(p_price) as price FROM tbl_cart WHERE usr_id = ?;'
        ...
        + ' WHEN y.pay_result = "refunded" THEN "환불완료" WHEN y.pay_result = "ready" THEN "미결제"'
        + ' ELSE "" END as payStatus'
        + ' FROM tbl_order_detail x, tbl_pay y WHERE x.order_no = y.order_no'
        ...
        + ' FROM tbl_pay WHERE pay_result = "paid" AND usr_id = ?;'
...
router.post('/opt', function(req, res) {
    ...
    conn.query('SELECT p_no as pNo, p_code as pCode, p_nm as pNm, p_price as pPrice, p_uniq_code as pUniqCode,'
        ...
        + ' opt_cnt as optCnt, sort_no as sortNo FROM tbl_product WHERE p_div = "O" AND p_uniq_code = ?;',
        [pUniqCode],
        function(err, results) {
            if(err) {
                console.log('err : ', err);
...
router.post('/cart/save', function(req, res) {
    ...
    if(pPairCodeYn == 'Y') {
        DEL_SQL1 = 'DELETE FROM tbl_cart WHERE p_code = "' + pPairCode + '" AND p_uniq_code = ? AND usr_id = ?;';
        console.log('>>> DEL_SQL1 : ', DEL_SQL1 + '\n');
    }

    var DEL_SQL2 = 'DELETE FROM tbl_cart WHERE p_code = ? AND p_uniq_code = ? AND usr_id = ?;';
    ...
    async.waterfall([
        ...
        function(callback) {
            ...
            // 전체 데이타를 조회한 후 결과를 'results' 매개변수에 저장.
            conn.connect();
            conn.query('DELETE FROM tbl_cart WHERE p_div = "M" AND p_uniq_code = ? AND usr_id = ?',
                [pUniqCode, ss.usrId],
                function (err, results) {
                    if (err) {
                        console.log('err : ', err);
        ...
        function(callback) {
            ...
            conn.connect();
            conn.query('INSERT INTO tbl_cart(usr_id, p_code, p_pair_code_yn, p_pair_code, p_div, p_type, p_gubun,'
                ...
                + '"' + ss.usrId + '" FROM tbl_product WHERE p_div = "M" AND p_uniq_code = ?',
                [pUniqCode],
                function (err, results) {
                    if (err) {
                        console.log('err : ', err);
        ...
        function(callback) {
            ...
            conn.query('INSERT INTO tbl_cart(usr_id, p_code, p_pair_code_yn, p_pair_code, p_div, p_type, p_gubun,'
                ...
                + 'SELECT "' + ss.usrId + '", p_code, p_pair_code_yn, p_pair_code, p_div, p_type, p_gubun,'
                + ' p_opt_btn_dis_yn, p_nm, p_price, p_uniq_code, sort_no, opt_cnt, "' + pCount + '", now(), "' + ss.usrId + '", now(),'
                + '"' + ss.usrId + '" FROM tbl_product WHERE p_div = "O" AND p_code = ? AND p_uniq_code = ?' ,
                [pCode, pUniqCode],
                function (err, results) {
                    if (err) {
                        console.log('err : ', err);
        ...
        function(callback) {
            ...
            conn.query('SELECT p_code as pCode, p_pair_code_yn as pPairCodeYn, p_pair_code as pPairCode, p_div as pDiv,'
                ...
                + ' p_opt_btn_dis_yn as pOptBtnDisYn, p_nm as pNm, p_price as pPrice, opt_cnt as optCnt, p_count as pCount,'
                + ' p_uniq_code as pUniqCode FROM tbl_cart WHERE usr_id = ? AND p_uniq_code = ? ORDER BY p_div ASC, sort_no ASC;',
                [ss.usrId, pUniqCode],
...
router.post('/cart/delete', function(req, res) {
    ...
    async.waterfall([
        function(callback) {
            ...
            if(pDiv == "M") {
                ...
                var conn = mysql.createConnection(config);
                conn.connect();
                conn.query('DELETE FROM tbl_cart WHERE p_uniq_code = ? AND usr_id = ?',
                    [pUniqCode, ss.usrId],
                    function(err, results) {
                        if(err) {
                            console.log('err : ', err);
    ...
    var conn = mysql.createConnection(config);
    conn.connect();
    conn.query('DELETE FROM tbl_cart WHERE p_code = ? AND p_uniq_code =?',
        [pCode, pUniqCode],
        function(err, results) {
            if(err) {
                console.log('err : ', err);
...
router.post('/order', function(req, res) {
    ...
    var categoryNo = req.body.categoryNo !=null ? req.body.categoryNo : '';
    var gameIdType = req.body.idType !=null ? req.body.idType : '';
    var gameUsrId = req.body.gameUsrId !=null ? req.body.gameUsrId : '';
    var gameUsrPwd = req.body.gameUsrPwd !=null ? req.body.gameUsrPwd : '';
    var gameCharNm = req.body.gameCharNm !=null ? req.body.gameCharNm : '';
    var gameLevelServer = req.body.gameLevelServer !=null ? req.body.gameLevelServer : '';
    var gameEtc = req.body.gameEtc !=null ? req.body.gameEtc : '';
    var codeType = req.body.codeType !=null ? req.body.codeType : '';
    var gameText = req.body.gameText !=null ? req.body.gameText : '';
    var starId = req.body.starId !=null ? req.body.starId : '';
    var starPwd = req.body.starPwd !=null ? req.body.starPwd : '';
    ...
    console.log('gameIdType : ', gameIdType + '\n');
    console.log('gameUsrId : ', gameUsrId + '\n');
    console.log('gameUsrPwd : ', gameUsrPwd + '\n');
    console.log('gameCharNm : ', gameCharNm + '\n');
    console.log('gameLevelServer : ', gameLevelServer + '\n');
    console.log('gameEtc : ', gameEtc + '\n');
    console.log('codeType : ', codeType + '\n');
    console.log('gameText : ', gameText + '\n');
    console.log('starId : ', starId + '\n');
    console.log('starPwd : ', starPwd + '\n');
    ...
    var orderNum = "ADPON" + cDate + randomString(6,"N");
    ...
    async.waterfall([
        function(callback) {
            if(prevOrderNo != '') {
                ...
                var conn = mysql.createConnection(config);
                conn.connect();
                conn.query('INSERT INTO tbl_pay_remain(order_no, usr_id, remain_days, used_days, remain_price,'
        ...
        function(callback) {
            // 기존 주문 히스토리에 복사.
            var conn = mysql.createConnection(config);
            conn.query('INSERT INTO  tbl_order_detail(`order_no`, `usr_id`, `p_code`, `p_div`, `p_pair_code_yn`,'
                + ' `p_pair_code`, `p_opt_btn_dis_yn`, `p_nm`, `p_price`, `p_disc_price`, `p_disc_percent`, `p_count`,'
                + ' `p_uniq_code`, `time`, `sort_no`,  `opt_cnt`,`p_type`, `insert_dt`, `insert_usr`, `update_dt`, `update_usr`)'
                + ' (SELECT `order_no`, `usr_id`, `p_code`, `p_div`, `p_pair_code_yn`, `p_pair_code`, `p_opt_btn_dis_yn`,'
                + ' `p_nm`, `p_price`, `p_disc_price`, `p_disc_percent`, `p_count`, `p_uniq_code`, `time`, `sort_no`,'
                + ' `opt_cnt`,`p_type`, `insert_dt`, `insert_usr`, `update_dt`, `update_usr`'
                + ' FROM tbl_order_detail WHERE p_uniq_code = ? AND usr_id = ?);'
                + 'INSERT INTO  tbl_order_his  (order_no, usr_id, service_nm, total_price, order_date, use_term_days,'
                ...
                + ' x.insert_usr, x.update_dt, x.update_usr FROM tbl_order x, tbl_order_detail y '
        ...
        function(callback) {
            ...
            // MySQL Connect
            var conn = mysql.createConnection(config);
            conn.query('DELETE FROM x USING tbl_order x, tbl_order_detail y'
                + ' WHERE x.order_no = y.order_no AND y.p_uniq_code = ? AND y.usr_id = ?;'
                + ' DELETE FROM tbl_order_detail WHERE p_uniq_code = ? AND usr_id = ?;',
                [productUniqCode, ss.usrId, productUniqCode, ss.usrId],
                function (err, results) {
                    if (err) {
                        console.log('err : ', err);
        ...
        function(callback) {
            ...
            // MySQL Connect
            var conn = mysql.createConnection(config);
            conn.query('INSERT INTO tbl_order_detail(order_no, usr_id, p_code, p_pair_code_yn, p_pair_code,'
                ...
                + 'FROM tbl_cart WHERE p_uniq_code = ? AND usr_id = ?',
        ...
        function(callback) {
            ...
            // 주문 처리.
            var conn = mysql.createConnection(config);
            conn.query('INSERT INTO tbl_order(order_no, usr_id, service_nm, total_price, order_date, use_term_days,'
                + ' category1, g_id_type, g_id, g_pwd, g_char_nm, g_leverl_server, g_etc, gc_type, gc_text, star_id, star_pwd,'
                + ' insert_dt, insert_usr, update_dt, update_usr)'
                + 'SELECT IFNULL(MAX("' + orderNum + '"),"' + orderNum + '"), "' + ss.usrId + '",'
                + ' (SELECT MAX(p_nm) as p_nm FROM tbl_cart WHERE p_code = ? AND p_div = "M" AND usr_id = ?),'
                + '"' + totalCalPayPrice + '", now(), "' + utDays + '",'
                + '"' + categoryNo + '", "' + gameIdType + '", "' + gameUsrId + '", "' + gameUsrPwd + '", "' + gameCharNm + '",'
                + '"' + gameLevelServer + '", "' + gameEtc + '", "' + codeType + '", "' + gameText + '", "' + starId + '", "' + starPwd + '",'
                + ' now(), "' + ss.usrId + '", now(), "' + ss.usrId
                + '" FROM tbl_cart WHERE p_code = ? AND p_div = "M" AND usr_id = ?;',
        ...
        function(callback) {
            ...
            conn.query('SELECT order_no as orderNo, p_code as pCode, p_pair_code_yn as pPairCodeYn, p_pair_code as pPairCode, p_div as pDiv,'
                ...
                + ' CASE WHEN p_div = "M" THEN "기본" WHEN p_div = "O" THEN "옵션" ELSE "" END as pDivNm,'
                + ' p_opt_btn_dis_yn as pOptBtnDisYn, p_nm as pNm, p_price as pPrice, opt_cnt as optCnt,'
                + ' p_uniq_code as pUniqCode FROM tbl_order_detail WHERE order_no = ? AND usr_id = ? ORDER BY p_code ASC, sort_no ASC;'
                + ' SELECT order_no as orderNo, service_nm as serviceNm, DATE_FORMAT(order_date, "%Y-%m-%d") as orderDate'
                + ' FROM tbl_order WHERE order_no = ? AND usr_id = ?;',
...
router.post('/order/delete', function(req, res) {
    ...
    if(pPairCodeYn == 'Y') {
        DEL_SQL1 = 'DELETE FROM tbl_order_detail WHERE p_code = "' + pPairCode + '" AND p_uniq_code = ? AND usr_id = ?;';
        console.log('>>> DEL_SQL1 : ', DEL_SQL1 + '\n');
    }

    var DEL_SQL2 = 'DELETE FROM tbl_order_detail WHERE p_code = ? AND p_uniq_code = ? AND usr_id = ?;';
    console.log('>>> DEL_SQL2 : ', DEL_SQL2 + '\n');
    ...
    async.waterfall([
        ...
        function(callback) {
            ...
            if(pDiv == "M") {
                ...
                var conn = mysql.createConnection(config);
                conn.connect();
                conn.query('DELETE FROM tbl_order_detail WHERE p_uniq_code = ? AND usr_id = ?',
                    [pUniqCode, ss.usrId],
                    function(err, results) {
                        if(err) {
                            console.log('err : ', err);
        ...
        function(callback) {
            ...
            var conn = mysql.createConnection(config);
            conn.connect();
            conn.query('DELETE FROM tbl_order WHERE order_no = ?',
                [pUniqCode, ss.usrId],
                function(err, results) {
                    if(err) {
                        console.log('err : ', err);
    ...
    var conn = mysql.createConnection(config);
    conn.connect();
    conn.query('DELETE FROM tbl_order_detail WHERE p_code = ? AND p_uniq_code =? AND order_no = ?',
...
router.post('/order/alldel', function(req, res) {
    ...
    async.waterfall([
        ...
        function(callback) {
            ...
            var conn = mysql.createConnection(config);
            conn.connect();
            conn.query('DELETE FROM tbl_order_detail WHERE usr_id = ?;',
                [ss.usrId],
                function(err, results) {
                    if(err) {
                        console.log('err : ', err);
        ...
        function(callback) {
            ...
            var conn = mysql.createConnection(config);
            conn.connect();
            conn.query('DELETE FROM tbl_order WHERE usr_id = ?;',
                [ss.usrId],
                function(err, results) {
                    if(err) {
                        console.log('err : ', err);
        ...
        function(callback) {
            ...
            conn.query('SELECT order_no as orderNo, p_code as pCode, p_pair_code_yn as pPairCodeYn, p_pair_code as pPairCode, p_div as pDiv,'
                + ' CASE WHEN p_div = "M" THEN "기본" WHEN p_div = "O" THEN "옵션" ELSE "" END as pDivNm,'
                + ' p_opt_btn_dis_yn as pOptBtnDisYn, p_nm as pNm, p_price as pPrice, opt_cnt as optCnt,'
                + ' p_uniq_code as pUniqCode FROM tbl_order_detail WHERE usr_id = ? ORDER BY p_code ASC, sort_no ASC;',
                [ss.usrId],
                function(err, results) {
                    if(err) {
                        console.log('err : ', err);
...
router.post('/calculate', function(req, res) {
    ...
    var pCodeCnt = req.body.pCodeCnt !=null ? req.body.pCodeCnt : 0;
    var termDays = req.body.termDays !=null ? req.body.termDays : 30;
    var pCount = req.body.pCount !=null ? req.body.pCount : 0;
    ...
    var pDiscPrice = req.body.pDisCountPrice !=null ? req.body.pDisCountPrice : 0;

    console.log('termDays : ', termDays);
    console.log('초기 prvRemainPrice : ', prvRemainPrice +'\n');
    console.log('pDiscPrice : ', pDiscPrice +'\n');
    ...
    if(pCodeCnt > 0 && pCodeCnt == 1) {

        console.log('pCode = ', pCode);
        console.log('pDiv = ', pDiv);
        console.log('pUniqCode = ', pUniqCode);
        console.log('pPrice = ', pPrice);
        console.log('pOptCnt = ', pOptCnt);

        if (pDiv == 'M') {
            getOptCnt += pOptCnt != null ? parseInt(pOptCnt) : 0;
            pCnt = pCount != null ? pCount : 0;
        ...
        calPrice += pPrice != null ? parseInt(pPrice) : 0;

    } else if (pCodeCnt > 0 && pCodeCnt > 1) {

        for (var x = 0; x < pCodeCnt; x++) {
            /*
             pCode[x] = req.body['pCode[]'] !=null ? req.body['pCode[]'] : '';
             pDiv[x] = req.body['pDiv[]'] != null ? req.body['pDiv[]'] : '';
             pUniqCode[x] = req.body['pUniqCode[]'] !=null ? req.body['pUniqCode[]'] : '';
             pPrice[x] = req.body['pPrice[]'] !=null ? req.body['pPrice[]'] : 0;
             pOptCnt[x] = req.body['pOptCnt[]'] !=null ? req.body['pOptCnt[]'] : 0;
             console.log('pCode [' + x + '] = ', pCode[x]);
             console.log('pDiv [' + x + '] = ', pDiv[x]);
             console.log('pUniqCode [' + x + '] = ', pUniqCode[x]);
             console.log('pPrice [' + x + '] = ', pPrice[x]);
             console.log('pOptCnt [' + x + '] = ', pOptCnt[x]);
    ...
    if(productType == "HDW") {
        disCountPrice = Math.abs(parseInt(pDiscPrice)) * pCnt;
        totalCalPrice = parseInt(calPrice);
        calPayPrice = Math.abs(parseInt(totalCalPrice)) - Math.abs(parseInt(disCountPrice));
...
router.get('/paypopup', function(req, res, next) {
    ...
    var SQL1 = 'SELECT p_no as pNo, p_code as pCode, p_nm as pNm, p_price as pPrice, p_uniq_code as pUniqCode, p_type as pType,'
        + ' p_desc as pDesc, opt_cnt as optCnt, p_count as pCount FROM tbl_product WHERE p_div = "M" AND p_display_yn = "Y";';
    // 주문 조회.
    /*
     ...
     + 'FROM tbl_order WHERE usr_id = ? ORDER BY order_date DESC;';
     ...
     + ' p_opt_btn_dis_yn as pOptBtnDisYn, opt_cnt as optCnt FROM tbl_order_detail WHERE usr_id = ? ORDER BY p_code ASC, sort_no ASC;';
     */
    var SQL2 = 'SELECT x.order_no as orderNo, x.usr_id as usrId, x.p_code as pCode, x.p_div as pDiv, x.p_type as pType,'
        ...
        + ' (SELECT total_price FROM tbl_order  WHERE order_no = x.order_no AND x.p_div = "M" AND x.usr_id = ?) as totalPrice,'
        + ' x.p_uniq_code as pUniqCode, x.p_opt_btn_dis_yn as pOptBtnDisYn, x.opt_cnt as optCnt, x.p_count as pCount'
        + ' FROM tbl_order_detail x, tbl_order y WHERE x.order_no = y.order_no AND y.pay_result IS NULL'
        + ' AND y.usr_id = ? ORDER BY x.p_code ASC, x.sort_no ASC;';

    // 카트 합계 조회.
    var SQL3 = 'SELECT SUM(total_price) as price FROM tbl_order WHERE pay_result IS NULL AND usr_id = ?;';
    // 이전 서비스 내역 조회.
    ...
    var SQL5 = 'SELECT order_no as orderNo, use_term_days as useTermDays, @rDays:=use_term_days, DATEDIFF(now(), pay_date) as usedDays,'
        ...
        + ' FROM tbl_pay WHERE pay_result = "paid" AND usr_id = ?;';
    // 주문자 정보 조회
    var SQL6 = 'SELECT c_id as cId, c_name as cName, c_email as cEmail, c_tel_no as cTelNo,'
        + ' c_tel_no1 as cTelNo1, c_tel_no2 as cTelNo2, c_tel_no3 as cTelNo3, c_cell_no as cCellNo, c_cell_no1 as cCellNo1,'
        + ' c_cell_no2 as cCellNo2, c_cell_no3 as cCellNo3 FROM tbl_user WHERE c_id = ?;';
...
router.post('/paypopup/list', function(req, res, next) {
    ...
    // 카트 합계 조회.
    var SQL2 = 'SELECT IFNULL(SUM(total_price),0) as price FROM tbl_order WHERE pay_result IS NULL AND usr_id = ?;';
...
router.post('/paypopup/alldel', function(req, res) {
    ...
    async.waterfall([
        ...
        function(callback) {
            ...
            var conn = mysql.createConnection(config);
            conn.connect();
            conn.query('DELETE FROM tbl_order_his WHERE usr_id = ?;',
                [ss.usrId],
                function(err, results) {
                    if(err) {
                        console.log('err : ', err);
        ...
        function(callback) {
            ...
            conn.query('SELECT x.order_no as orderNo, x.usr_id as usrId, x.p_code as pCode, x.p_div as pDiv, x.p_type as pType,'
                ...
                + ' (SELECT total_price FROM tbl_order  WHERE order_no = x.order_no AND x.p_div = "M" AND x.usr_id = ?) as totalPrice,'
                + ' x.p_uniq_code as pUniqCode, x.p_opt_btn_dis_yn as pOptBtnDisYn, x.opt_cnt as optCnt, x.p_count as pCount'
                + ' FROM tbl_order_detail x, tbl_order y WHERE x.order_no = y.order_no AND y.usr_id = ? ORDER BY x.p_code ASC, x.sort_no ASC;'
                + ' SELECT IFNULL(SUM(total_price),0) as totalPrice FROM tbl_order WHERE usr_id = ?;',
...
router.post('/paypopup/delete', function(req, res) {
    ...
    async.waterfall([
        ...
        function(callback) {
            ...
            var conn = mysql.createConnection(config);
            conn.connect();
            conn.query('DELETE FROM tbl_order WHERE order_no = ? AND usr_id = ?;',
                [orderNo, ss.usrId],
                function(err, results) {
                    if(err) {
                        console.log('err : ', err);
        ...
        function(callback) {
            ...
            var conn = mysql.createConnection(config);
            conn.connect();
            conn.query('DELETE FROM tbl_order_his WHERE order_no = ? AND usr_id = ?;',
                [orderNo, ss.usrId],
                function(err, results) {
                    if(err) {
                        console.log('err : ', err);
        ...
        function(callback) {
            ...
            conn.query('SELECT x.order_no as orderNo, x.usr_id as usrId, x.p_code as pCode, x.p_div as pDiv, x.p_type as pType,'
                ...
                + ' FROM tbl_order_detail x, tbl_order y WHERE x.order_no = y.order_no AND y.pay_result IS NULL'
                + ' AND y.usr_id = ? ORDER BY x.p_code ASC, x.sort_no ASC;'
                + ' SELECT IFNULL(SUM(total_price),0) as totalPrice FROM tbl_order WHERE pay_result IS NULL AND usr_id = ?;',
                [ss.usrId, ss.usrId, ss.usrId],
                function(err, results) {
                    if(err) {
                        console.log('err : ', err);
